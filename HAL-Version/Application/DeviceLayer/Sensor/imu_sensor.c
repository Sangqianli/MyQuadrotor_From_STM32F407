...
static void imu_sensor_check(imu_sensor_t *imu_sen)
{
	static float roll_rec;
    imu_sensor_info_t *imu_info = imu_sen->info;
    // ¼ÓÈëÂË²¨Ëã·¨

	if(fabs(roll_rec - imu_info->roll) >=340)
		imu_info->roll -= 360;
	
	roll_rec = imu_info->roll;
	
    imu_info->rate_yaw -= imu_info->rate_yaw_offset;
    imu_info->rate_pitch -= imu_info->rate_pitch_offset;
	
	imu_info->pitch -= imu_info->angle_pitch_offset;
	imu_info->yaw -= imu_info->angle_yaw_offset;
	imu_info->roll -= imu_info->angle_roll_offset;
	

	roll_rec2 = imu_info->roll;
